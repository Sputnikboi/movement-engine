...
void main() {
    ...
    // Emissive flag: normal_len < 0.5 means self-lit (no lighting/specular).
    // normal.x encodes alpha (0 = use 1.0, otherwise use the value directly).
    if (normal_len < 0.5) {
        vec3 color = frag_color;

        // Alpha: stored in normal.x (0 means fully opaque)
        float alpha = (frag_normal.x > 0.001) ? frag_normal.x : 1.0;
